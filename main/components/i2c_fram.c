...
void i2c_fram_read(uint32_t addr, uint8_t *buf, uint32_t sz)
{
	...
	ESP_ERROR_CHECK(i2c_master_write_read_device(0, I2C_FM24LC16B_ADDR + (addr >> 8), &addr_lo, 1, buf, sz, 2));
}
...
void i2c_fram_write(uint32_t addr, const uint8_t *buf, uint32_t sz)
{
	...
	b[0] = addr & 0xFF;
	memcpy(&b[1], buf, sz);
	ESP_ERROR_CHECK(i2c_master_write_to_device(0, I2C_FM24LC16B_ADDR + (addr >> 8), b, 1 + sz, 2));
}
